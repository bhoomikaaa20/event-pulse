...
function EventDetail() {
  ...
  useEffect(() => {
    ...
    supabase.rpc("increment_view", { _event_id: eventId }).then(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      if (data.user) {
        ...
        const { data: l } = await supabase
          .from("event_likes").select("id").eq("event_id", eventId).eq("user_id", data.user.id).maybeSingle();
        setLiked(!!l);
      }
    });
  }, [eventId, qc]);
  ...
  const handleLike = async () => {
    ...
    const { data, error } = await supabase.rpc("toggle_like", { _event_id: eventId });
